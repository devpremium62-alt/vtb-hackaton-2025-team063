...
const SwipeForDelete = ({onDelete, children}: Props) => {
    ...
    return <>
        <motion.div
            className={`absolute right-0 top-0 bottom-0 rounded-xl w-[40px] bg-error flex items-center justify-center text-white ${open ? "pointer-events-auto" : "pointer-events-none"}`}
            initial={{opacity: 0}}
