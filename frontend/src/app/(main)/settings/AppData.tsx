...
import DeleteDataModal from "@/app/(main)/settings/DeleteDataModal";
    ...
    return <section className={`${className} mb-[1.875rem]`}>
        ...
        <div className="bg-tertiary rounded-xl px-2.5 py-1.5">
            <motion.div
                ...
                <AccentButton onClick={() => setModalActive(true)} className="w-full justify-center gap-1 py-1.5"
                              background="bg-error" type="submit">
                    <Delete/>
                    Очистить данные
                </AccentButton>
            </motion.div>
        </div>
        <DeleteDataModal isActive={isModalActive} setActive={setModalActive}/>
    </section>
}
